...
export function FloatingActions({ onWishlistClick }: FloatingActionsProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      text: "Hi! Welcome to VisionFlix Streams. How can I help you today?",
      isUser: false,
      timestamp: new Date()
    }
  ])
      ...
      <div className="fixed bottom-5 left-5 z-[10000] flex flex-col gap-4">
        ...
        <div className="relative">
          <motion.button
            ...
            className="flex items-center justify-center w-14 h-14 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 text-white rounded-full shadow-lg shadow-red-500/30 transition-colors relative z-10"
            ...
            {isChatOpen && (
              ...
              >
                <div className="backdrop-blur-xl bg-black/80 border border-white/20 rounded-2xl shadow-2xl overflow-hidden flex flex-col w-full h-full">
                  {/* Header */}
                  <div className="bg-gradient-to-r from-red-600 to-red-500 p-3 md:p-4 relative flex-shrink-0">
                    <button
        ...
        <motion.button
          ...
          className="relative flex items-center justify-center w-14 h-14 bg-red-500 text-white rounded-full shadow-lg hover:bg-red-600 transition-colors"
        >
          <Heart className="w-6 h-6" />
          {itemCount > 0 && (
            <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-white text-[10px] font-bold text-red-600 shadow-md">
              {itemCount}
            </span>
          )}
        </motion.button>
