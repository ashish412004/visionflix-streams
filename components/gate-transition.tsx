...
export function GateTransition() {
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 bg-gradient-to-r from-purple-600/40 to-pink-600/40 border border-purple-500/60 rounded-full px-6 py-3 backdrop-blur-lg">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center">
            <Shield className="w-5 h-5 text-white" />
          </div>
          <span className="text-pink-300 font-bold tracking-wider text-lg">INITIATORS</span>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 blur-3xl"
      />
    </div>
  )
}
